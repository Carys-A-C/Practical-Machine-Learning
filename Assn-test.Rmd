...
This report briefly summarizes the approaches taken in order to understand, explore and simplify the dataset as well as the models fitted.


#### Data Exploration

The data was first loaded into R. Any data marked as "NA", empty strings (""), and the excell expression "#DIV/0!" (indicating an infinite value, or divisiion by zero) were all set to be NA. The seed was set to 22100 for reproducibility.
...
I digress, I split the data into two parts, training and test, with 70% of the data in the training set.
```{r read-and-split}
library(caret)
library(tidyverse)
library(factoextra)
...
setwd("/Users/caryscroft/Desktop/RCoursera/Practical Machine Learning/")
...
```{r exploration-density, fig.width = 20, fig.height = 60}
...
ggplot(melt.train, aes(x = value, colour = classe)) + 
    geom_density(lwd = 2) + facet_wrap(~variable, scales = "free", ncol = 4) + theme_bw()
```
